    ...
    <div className="admin-page p-6 lg:p-10 space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-slate-400">{filtered.length} contactos</p>
        </div>
      ...
      <div className="admin-card overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-500 gap-4">
            ...
            <p className="text-sm font-medium">No se encontraron clientes que coincidan con la búsqueda.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800/50 bg-slate-900/50">
                  ...
                  <th className="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Contacto</th>
                  ...
                  <th className="py-4 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {filtered.map((client) => {
                  const s = statusMap[client.status] || { label: client.status || '—', cls: 'bg-slate-800 text-slate-300 border-slate-700' };
                  return (
                    <tr key={client.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-semibold text-slate-200">{client.full_name}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex justify-center gap-2">
                          <button className="p-2 text-slate-400 hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 rounded-lg transition-colors" title="Ver">
                            <Eye size={18} />
                          </button>
                          <button className="p-2 text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-colors" title="Editar">
                            <Edit2 size={18} />
                          </button>
                        </div>
                      </td>
