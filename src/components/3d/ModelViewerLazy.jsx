...
export default function ModelViewerLazy({ modelUrl, projectId, canEdit = false, onModelUrlChange, contactPhone, projectTitle }) {
  ...
  const handleUpload = useCallback(async (file) => {
    ...
    try {
      ...
      // 5. Send SMS notification to project contact
      if (contactPhone) {
        try {
          const smsBody = `🏗️ Barba Construction: ¡Tu propuesta 3D para "${projectTitle || 'tu proyecto'}" está lista! Ingresa a tu portal de cliente para explorar el modelo interactivo en 3D. 🎉`;
