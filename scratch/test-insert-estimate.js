...
async function testInsert() {
  try {
    ...
    // 3. Insert dummy estimate item with the same payload we use in Estimator.jsx and ReceiptSidebar.jsx
    const { data: item, error: itemError } = await supabase
      ...
      .insert([{
        ...
        quantity: 10,
        unit_price: 15,
        total: 150, // Wait, this column doesn't exist in SQL, does it?
        service_type: 'gutters' // Wait, this column doesn't exist in SQL, does it?
      }])
      .select();
    ...
    if (itemError) {
      ...
      console.log("Estimate item inserted successfully:", item);
    }
