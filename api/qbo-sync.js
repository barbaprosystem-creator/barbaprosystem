...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!settings.qbo_refresh_token) {
      return res.status(400).json({ error: 'QuickBooks is not connected. Please connect it in settings.' });
    }
    ...
    if (!contact) {
      return res.status(400).json({ error: 'Estimate has no associated client contact' });
    }
    ...
    const invoiceLineItems = (estimateItems || []).map(item => {
      ...
      return {
        ...
        SalesItemLineDetail: {
          ItemRef: {
            value: '1', // Fallback default service item ID in QuickBooks Online
            name: 'Services'
          },
